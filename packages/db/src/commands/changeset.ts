...
export async function applyChangeSet(
  ...
  const cs = await prisma.changeSet.findUnique({ where: { id: input.changeSetId } });
  ...
  const updated = await prisma.changeSet.updateMany({
  ...
  const next = await prisma.changeSet.findUniqueOrThrow({ where: { id: cs.id } });
